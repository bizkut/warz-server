...
public class SteamApi
{
    ...
    public bool CheckAppOwnership(string steamId, string appId)
    {
        Dictionary<string, string> prm = new Dictionary<string, string>();
        prm.Add("format", "xml");
        ...
        try
        {
            string ans = HttpPost("ISteamUser/CheckAppOwnership/V0001/", prm, false);
            
            try
            {
                XmlSerializer xs = new XmlSerializer(typeof(SteamXML.CheckAppOwnershipResponse));
                MemoryStream memoryStream = new MemoryStream(WebHelper.StringToUTF8ByteArray(ans));
                SteamXML.CheckAppOwnershipResponse resp = (SteamXML.CheckAppOwnershipResponse)xs.Deserialize(memoryStream);
                return resp.ownsapp && resp.permanent;
            }
            catch (Exception e)
            {
                throw e;
        ...
        {
            // steam return 405 - not authorized if we don't have app ownership for 
            return false;
        }
    }

    public SteamXML.appownershipAppsApp[] GetPublisherAppOwnership(string steamId, ref string apiAnswer)
    {
        Dictionary<string, string> prm = new Dictionary<string, string>();
        prm.Add("format", "xml");
        prm.Add("steamid", steamId);
        ...
        try
        {
            string ans = HttpPost("ISteamUser/GetPublisherAppOwnership/V0001/", prm, false);
            apiAnswer = ans;

            try
            {
                XmlSerializer xs = new XmlSerializer(typeof(SteamXML.GetPublisherAppOwnershipResponse));
                MemoryStream memoryStream = new MemoryStream(WebHelper.StringToUTF8ByteArray(ans));
                SteamXML.GetPublisherAppOwnershipResponse resp = (SteamXML.GetPublisherAppOwnershipResponse)xs.Deserialize(memoryStream);
                if (resp.apps == null)
                    return null;
